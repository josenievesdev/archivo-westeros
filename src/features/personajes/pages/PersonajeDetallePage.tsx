...
export function PersonajeDetallePage() {
  ...
  const view = createCharacterViewModel(character.data)
    ...
    <article className="max-w-4xl space-y-8 sm:space-y-10">
      ...
      <Surface className="p-5 sm:p-7">
        <dl className="grid gap-0 sm:grid-cols-2">
          {[
            ['Alias', view.aliases.map((alias) => alias.value).join(', ') || 'No indicados'],
            ['Títulos', view.titles.map((title) => title.value).join(', ') || 'No indicados'],
            ['Nacimiento', view.born?.value || 'No indicado'],
            ['Interpretado por', view.playedBy.join(', ') || 'No indicado'],
          ].map(([term, value]) => (
