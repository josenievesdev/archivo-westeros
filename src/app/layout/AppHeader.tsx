...
export function AppHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 h-16 border-b border-etch bg-[#0b0d11f2] backdrop-blur-md md:h-20">
      <div className="relative mx-auto flex h-full w-full max-w-[90rem] items-center justify-between gap-6 px-5 sm:px-8 xl:px-12">
        ...
        >
          ...
          <span className="min-w-0">
            <span className="block truncate font-display text-[0.8125rem] font-semibold uppercase tracking-[0.08em] text-bone xl:text-[0.9375rem] xl:tracking-[0.226em]">
              Archivo de Westeros
            </span>
            <span className="mt-px hidden font-sans text-[0.59375rem] tracking-[0.1rem] text-ash lg:block">
            ...
            {desktopNavigation.map((item) => (
              <li key={item.to}>
                <NavLink
                  className={({ isActive }) =>
                    cx(
                      'relative flex min-h-11 items-center font-sans text-[0.78125rem] tracking-[0.09375rem] transition-colors after:absolute after:right-1/2 after:bottom-2.5 after:h-px after:w-0 after:translate-x-1/2 after:bg-gold after:transition-[width] hover:text-bone',
